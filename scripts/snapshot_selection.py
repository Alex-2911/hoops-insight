...
def resolve_snapshot_selection(source_root: Path) -> SnapshotSelection:
    ...
    for snapshot_date, combined in sorted(combined_candidates, key=lambda item: item[0], reverse=True):
        local_matched = _exact_dated(lightgbm, "local_matched_games_", snapshot_date, ".csv")
        if local_matched is None:
            selection_errors.append(f"{snapshot_date}: local_matched_games missing")
            continue
        ...
        if strategy_path is None:
            selection_errors.append(f"{snapshot_date}: strategy_params missing")
            continue
        strategy_date = (
        ...
        if strategy_date != snapshot_date:
            selection_errors.append(
                f"{snapshot_date}: strategy_params date {strategy_date or 'unknown'} mismatch ({strategy_path.name})"
            )
            continue
        ...
        if metrics_path is None:
            selection_errors.append(f"{snapshot_date}: metrics_snapshot missing")
            continue
        metrics_date = _extract_date_from_name(metrics_path.name) or _extract_date_from_json(metrics_path)
        if metrics_date != snapshot_date:
            selection_errors.append(
                f"{snapshot_date}: metrics_snapshot date {metrics_date or 'unknown'} mismatch ({metrics_path.name})"
            )
            continue
        ...
        if bet_log_path is None:
            selection_errors.append(f"{snapshot_date}: bet_log_flat_live missing")
            continue
        bet_log_date = _extract_date_from_name(bet_log_path.name) or _extract_max_date_from_csv(bet_log_path)
        if bet_log_date != snapshot_date:
            selection_errors.append(
                f"{snapshot_date}: bet_log_flat_live date {bet_log_date or 'unknown'} mismatch ({bet_log_path.name})"
            )
            continue
